...
function seedDemoFirm() {
  ...
  // Populate realistic demo leads so the dashboard looks lived-in immediately.
  // Disable by setting DEMO_SEED=off (e.g. when using a real database for the demo).
  if (process.env.DEMO_SEED !== 'off' && !mem.leads.some((l) => l.firmSlug === 'demo')) {
    const { demoLeads } = require('./demoSeed');
    mem.leads.unshift(...demoLeads('demo'));
  }
}
...
async function addLeadNote(id, firmSlug, note) {
  const entry = { text: note, createdAt: new Date().toISOString() };
  const sb = getSupabase();
  if (sb) {
    // Append to the notes jsonb array
    const { data: existing } = await sb.from('leads').select('notes').eq('id', id).eq('firmSlug', firmSlug).single();
    const notes = [...(existing?.notes || []), entry];
    const { data, error } = await sb.from('leads').update({ notes }).eq('id', id).eq('firmSlug', firmSlug).select().single();
    if (error) throw new Error(`store.addLeadNote: ${error.message}`);
    return data;
  }
  const lead = mem.leads.find((l) => l.id === id && l.firmSlug === firmSlug);
  if (!lead) return null;
...
async function updateFirm(slug, updates) {
  const allowed = ['name', 'practiceAreas', 'jurisdiction', 'notifyEmail', 'alertWebhook', 'crm'];
  const safe = Object.fromEntries(Object.entries(updates).filter(([k]) => allowed.includes(k)));
  const sb = getSupabase();
  if (sb) {
    const { data, error } = await sb.from('firms').update(safe).eq('slug', slug).select().single();
    if (error) throw new Error(`store.updateFirm: ${error.message}`);
    return data;
  }
  seedDemoFirm();
  const firm = mem.firms.get(slug);
  if (!firm) return null;
  Object.assign(firm, safe);
  return firm;
}

// ---- Admin queries ----

async function listAllFirms() {
  const sb = getSupabase();
  if (sb) {
    const { data, error } = await sb.from('firms').select('*').order('created_at', { ascending: false });
    if (error) throw new Error(`store.listAllFirms: ${error.message}`);
    return data || [];
  }
  seedDemoFirm();
  return [...mem.firms.values()];
}
