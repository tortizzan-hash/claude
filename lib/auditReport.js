...
function generateReport(firm, leads) {
  ...
  return {
    ...
    summary: {
      ...
      highSignal: bandCounts.priority.count,
...
function conversionByBand(leads) {
  ...
  // Lift: priority-band rate vs low-band rate
  const highRate = out.priority.rate;
...
function missedOpportunities(leads) {
  ...
  const stale = leads.filter((l) => {
    if (l.band !== 'priority' && !l.forceReview) return false;
...
function intakeHealth(leads) {
  ...
  const highPct = pct(leads.filter((l) => l.band === 'priority').length, total);
  const stalePct = pct(
    leads.filter((l) => l.disposition === 'new' && ageHours(l) > 24).length,
    total
  );
  const deadHighPct = pct(
    leads.filter((l) => l.band === 'priority' && l.disposition === 'dead').length,
    leads.filter((l) => l.band === 'priority').length || 1
  );
  const convRate = pct(
    leads.filter((l) => l.disposition === 'booked' || l.disposition === 'signed').length,
    total
  ...
  if (deadHighPct > 20) issues.push(`${deadHighPct}% of priority leads went cold — qualification is working but follow-up is not.`);
  if (noContactPct > 40) issues.push(`${noContactPct}% of leads have no phone or email — intake form is not capturing contact info.`);
  if (highPct < 10) issues.push(`Only ${highPct}% of leads are priority — either traffic quality is low or the intake form is not filtering well.`);

  if (convRate > 30) strengths.push(`${convRate}% overall conversion rate — above the industry average of 14%.`);
  if (highPct > 25) strengths.push(`${highPct}% of leads are priority — strong lead quality.`);
...
function buildRecommendations({ leads, bandCounts, conversion, missed, health }) {
  ...
  if (conversion.priority.rate < 40 && conversion.priority.total > 0) {
    recs.push({
      priority: 'high',
      title: 'Improve follow-up on priority leads',
      detail: `You're converting ${conversion.priority.rate}% of priority leads. A structured follow-up sequence (call → SMS → email) typically reaches 40%+.`,
    });
  }
  ...
  if (bandCounts.low.count > bandCounts.priority.count * 2) {
    recs.push({
      priority: 'medium',
