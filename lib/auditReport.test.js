...
const leads = [
  { lqs: 72, band: 'high', label: 'High Signal', disposition: 'signed', contact: { phone: '555-1111' }, overrides: [], forceReview: false, createdAt: new Date(Date.now() - 10000).toISOString(), message: 'rear-ended last week' },
  { lqs: 68, band: 'high', label: 'High Signal', disposition: 'dead', contact: { phone: '555-2222' }, overrides: [], forceReview: false, createdAt: new Date(Date.now() - 1000 * 60 * 60 * 72).toISOString(), message: 'slipped and fell' },
...
test('band distribution counts are correct', () => {
  ...
  assert.strictEqual(bd.high.count, 2);
...
test('conversion rates compute per band', () => {
  ...
  assert.strictEqual(cv.high.total, 2);
  assert.strictEqual(cv.high.converted, 1); // only signed counts
  assert.strictEqual(cv.high.rate, 50);
  assert.strictEqual(cv.qualified.converted, 1);
});

test('missed opportunities catches dead high-signal + stale', () => {
  const { missedOpportunities: mo } = generateReport(firm, leads);
  // lead[1]: high-signal, dead = missed. lead[3]: low-band, excluded.
