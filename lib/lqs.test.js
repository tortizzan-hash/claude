...
// NOTE: the multiplicative model is intentionally strict — only a near-perfect
// lead reaches the "high" band. These tests document the CURRENT (canon) band
// calibration. If VML decides to recalibrate bands, update these expectations.
test('realistic: near-perfect inquiry reaches high band (~73.5)', () => {
  const lqs = computeLqs({ iss: 95, cfs: 95, bis: 95, frs: 90, crs: 5 });
  assert.ok(lqs >= 70, `expected high band, got ${lqs}`);
  assert.strictEqual(bandFor(lqs).band, 'high');
});

test('realistic: strong auto-accident inquiry lands qualified (~60.8)', () => {
  // Clear facts, right practice area, ready to hire, low friction — yet still
  // "qualified" not "high" under the strict multiplicative model.
  const lqs = computeLqs({ iss: 92, cfs: 95, bis: 90, frs: 85, crs: 10 });
  assert.ok(lqs >= 45 && lqs < 70, `expected qualified band, got ${lqs}`);
  assert.strictEqual(bandFor(lqs).band, 'qualified');
});
...
test('realistic: good signal but wrong jurisdiction (low CFS) gets penalized', () => {
  ...
  assert.ok(lqs < 45, `weak case fit should prevent qualified+, got ${lqs}`);
});
