...
const BANDS = ['high', 'qualified', 'review', 'low'];
...
function computeStats(leads = []) {
  const summary = {
    ...
    highSignal: leads.filter((l) => l.band === 'high').length,
  };
  ...
  // The headline proof line: do high-band leads convert better than low-band?
  const high = perBand.high.conversionRate;
